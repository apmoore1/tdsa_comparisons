...
do
    ...
    do
        ...
        model_save_name=$model"_"$domain"_baseline"
        $python_path ./tdsa_comparisons/experiments/flexi_run_models.py "${data_dir}" "${config_fp}" $number_runs $domain $model $model_dir $model_save_name --glove
    done
done